...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-04-09 | 目录结构、配置文件、main.py 已创建 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-04-09 | 27个冒烟测试全绿 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-04-09 | Settings+load_settings+validate+11单元测试 |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-04-09 | BaseLLM+ChatMessage+ChatResponse+LLMFactory+12单元测试 |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-04-09 | BaseEmbedding+EmbeddingFactory+11单元测试 |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-04-09 | BaseSplitter+SplitterFactory+5单元测试 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| C1 | 定义核心数据类型/契约（Document/Chunk/ChunkRecord） | [x] | 2026-04-10 | ImageRef+Document+Chunk+ChunkRecord+22单元测试 |
| C2 | 文件完整性检查（SHA256） | [x] | 2026-04-10 | FileIntegrityChecker+SQLiteIntegrityChecker(WAL)+13单元测试 |
...
| H4 | 评估面板页面 | [x] | 2026-04-12 | evaluation_panel页+运行评估+指标展示+历史对比 |
| H5 | Recall 回归测试（E2E） | [x] | 2026-04-12 | test_recall E2E+hit_rate阈值+4测试用例 |

#### 阶段 I：端到端验收与文档收口

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| I1 | E2E：MCP Client 侧调用模拟 | [x] | 2026-04-12 | test_mcp_client+协议模拟+子进程启动+14测试用例 |
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-04-12 | AppTest冒烟测试6页+修复overview secrets+7测试用例 |
| I3 | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-04-12 | README补齐配置说明+MCP示例+Dashboard指南+测试命令 |
| I4 | 清理接口一致性（契约测试补齐） | [ ] | | |
| I5 | 全链路 E2E 验收 | [ ] | | |
